"""Módulo para gerenciar o jogador e seus movimentos."""
...
def iniciar_jogador(labirinto):
    ...
    """
    return (1, 1), 0  # Posição inicial e pontuação
    ...
    def on_press(key):
        ...
        if (0 <= nova_pos[0] < len(labirinto) and 
            ...
            if labirinto[nova_pos[0]][nova_pos[1]] == "*":
                nova_pontuacao += 10
                labirinto[nova_pos[0]][nova_pos[1]] = " "  # Remove item
        else:
            nova_pos = pos_atual  # Volta se for parede ou fora do limite

    ouvinte = keyboard.Listener(on_press=on_press)
    ouvinte.start()
    ouvinte.join(timeout=0.1)  # Aguarda entrada por 0.1s
